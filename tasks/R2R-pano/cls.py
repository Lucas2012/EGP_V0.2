...
class CLS(object):
  ...
  def __init__(self, graphs, weight='weight', threshold=3.0):
    ...
    for scan, graph in self.graphs.items():
        self.distances[scan] = dict(nx.all_pairs_dijkstra_path_length(graph, weight=self.weight))

  def __call__(self, scan, prediction, reference):
    ...
    def length(nodes):
      return np.sum([
          self.graphs[scan].edges[edge].get(self.weight, 1.0)
          for edge in zip(nodes[:-1], nodes[1:])
      ])

    coverage = np.mean([
        np.exp(-np.min([  # pylint: disable=g-complex-comprehension
            self.distances[scan][u][v] for v in prediction

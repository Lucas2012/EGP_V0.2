...
from SF_utils.SF_utils import vocab_pad_idx, vocab_bos_idx, vocab_eos_idx, flatten, try_cuda
from SF_utils.SF_follower import batch_instructions_from_encoded
...
class Seq2SeqSpeaker(object):
    ...
    def _score_obs_actions_and_instructions(self, start_obs, batched_image_features, batched_action_embeddings, \
                                            path_mask, path_lengths, perm_indices, encoded_instructions, feedback):
        assert len(start_obs) == len(batched_action_embeddings)
    ...
    def _batch_observations_and_actions_PanoBatch(self, all_action_embeddings, all_feature_embeddings, encoded_instructions):
        seq_lengths = np.array([len(a) for a in all_action_embeddings])
        max_path_length = seq_lengths.max()

        # DO NOT permute the sequence, since here we are doing manual LSTM unrolling in encoder
        perm_indices = np.arange(len(all_action_embeddings))

        batch_size = len(all_feature_embeddings)
        assert batch_size == len(all_action_embeddings)

        mask = np.ones((batch_size, max_path_length), np.uint8)
        # action embedding tensor
        action_embedding_dim = all_action_embeddings[0][0].shape[-1]
        batched_action_embeddings = [
            np.zeros((batch_size, action_embedding_dim), np.float32)
            for _ in range(max_path_length)]

        image_feature_shape = all_feature_embeddings[0][0].shape
        # image embedding tensor
        batched_image_features = [
            np.zeros((batch_size,) + image_feature_shape, np.float32)
            for _ in range(max_path_length)]

        for i, (feature_embeddings, action_embeddings) in enumerate(zip(all_feature_embeddings, all_action_embeddings)):
            # don't include the last state, which should result after the stop action
            mask[i, :len(action_embeddings)] = 0
            for t, (feature, action) in enumerate(zip(feature_embeddings, action_embeddings)):
                batched_image_features[t][i]    = feature
                batched_action_embeddings[t][i] = action
    ...
    def _score_obs_actions_and_instructions_PanoBatch(self, start_obs, batched_image_features, batched_action_embeddings, \
                                                            encoded_instructions, feedback):
        assert len(start_obs) == len(batched_action_embeddings)
        batched_image_features, batched_action_embeddings, path_mask, \
            path_lengths, encoded_instructions, perm_indices = \
                 self._batch_observations_and_actions_PanoBatch(
                     batched_action_embeddings, batched_image_features, encoded_instructions)
    ...
    def rollout(self, info_list):
        start_obs, batched_image_features, batched_action_embeddings = info_list
        encoded_instructions = [[] for _ in range(len(start_obs))]
        outputs, loss = self._score_obs_actions_and_instructions_PanoBatch(start_obs, batched_image_features, batched_action_embeddings, \
                                                                           encoded_instructions, self.feedback)
        return outputs
    ...
    def test(self, use_dropout=False, feedback='argmax', info_list=[], beam_size=1):
        ...
        rollout_results = self.rollout(info_list)

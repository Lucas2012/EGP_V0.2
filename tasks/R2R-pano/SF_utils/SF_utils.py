...
def load_datasets(splits, dataset_name='R2R'):
    if dataset_name not in ['R2R', 'R4R']:
        raise ValueError("Dataset name must be either 'R2R' or 'R4R'")
    prefix='tasks/R2R/data/{}'.format(dataset_name)
    data = []
    for split in splits:
        assert split in ['train', 'val_seen', 'val_unseen', 'test', 'train_val_seen', 'synthetic', 'data_augmentation_paths']
        if split == 'synthetic':
            if dataset_name == 'R4R':

...
def load_datasets(splits, fraction=1.0, dataset_name='R2R'):
    data = []
    #file_path = os.path.dirname(__file__)
    file_path ='tasks/R2R-pano/data/{}'.format(dataset_name)
    for split in splits:
        #_path = os.path.abspath(os.path.join(file_path,'data','R2R_%s.json' % split))
        _path = os.path.abspath(file_path + '_%s.json' % split)
        with open(_path) as f:
            loaded_data = json.load(f)
            if fraction < 1.0:
                loaded_data = loaded_data[:int(fraction * len(loaded_data))]
            data += loaded_data
    return data

...
class PanoSeq2SeqTrainer():
    ...
    def train(self, epoch, train_env, tb_logger=None):
        ...
        for iter in range(1, self.train_iters_epoch + 1):
            ...
            if self.opts.arch == 'cogrounding':
                ...
                loss, traj = self.agent.rollout_cogrounding()
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t  TranslatorTau {tau:.4f} \t'.format(
                epoch, iter, self.train_iters_epoch, batch_time=batch_time,
                loss=losses, tau=self.agent.translator.tau if self.opts.use_translator else 0))
    ...
    def eval(self, epoch, val_env, val_loader=None, use_loader=False, tb_logger=None):
        batch_time = AverageMeter()
        losses = AverageMeter()
        dists = AverageMeter()
        movements = AverageMeter()
